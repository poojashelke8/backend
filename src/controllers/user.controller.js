...
const updateUserAvatar = asyncHandler(async(req,res)=>{
    ...
    const avatarLocalPath = req.file?.avatar[0]?.path?.replace(/\\/g, '/');
    if(!avatarLocalPath){
...
const updateUserCoverImage = asyncHandler(async(req,res)=>{
    ...
    const coverImageLocalPath = req.file?.coverImage[0]?.path?.replace(/\\/g, '/');
...
const getWatchHistory = asyncHandler(async(req,res)=>{
    // in req.user._id returns string as it stroed as a string in mongoDb
    const user = await User.aggregate([
        {
            $match:{
                // to access the _id cannot pass directly as its string, 
                // using below we can directly pass the id
